...
if (!passportHarvest) {
  ...
  const photoActivities = activities.filter((activity) => (activity.type || 'text') === 'photo' && CropPassportPhoto.isPhotoData(activity.photoData));
  const voiceActivities = activities.filter((activity) => (activity.type || 'text') === 'voice' && CropPassportVoice.isAudioData(activity.audioData));
  ...
  if (photoActivities.length) document.querySelector('.timeline-section').insertAdjacentHTML('afterend', CropPassportPhoto.renderGallery(photoActivities, escapeHtml));
  if (voiceActivities.length) (document.querySelector('.photo-gallery-section') || document.querySelector('.timeline-section')).insertAdjacentHTML('afterend', CropPassportVoice.renderNotes(voiceActivities, escapeHtml));
  CropPassportVoice.hydratePlayers(passportContent);
