...
function saveWithoutAi() { if (!activityForm.reportValidity()) return; saveOriginalActivity(getFormActivity()); activityForm.reset(); closeActivityModal(); renderActivities(); }

async function submitForAiReview(event) {
  event.preventDefault(); if (!activityForm.reportValidity()) return;
  const formData = getFormActivity(); pendingActivity = formData; closeActivityModal(); resetReviewActions(); aiReviewLoading.hidden = false; aiReviewResults.hidden = true; aiReviewActions.hidden = true; aiReviewModal.showModal();
  try {
    ...
    aiReviewLoading.hidden = true; aiReviewResults.hidden = false; document.querySelector('#ai-category').textContent = 'AI Analysis Unavailable'; document.querySelector('#ai-summary').textContent = 'AI analysis is temporarily unavailable. You can still save your original farming record.'; document.querySelector('#ai-attention-level').textContent = 'Not available'; document.querySelector('#ai-follow-up').textContent = 'Save the original note now or return to your note and try again later.'; document.querySelector('#ai-original-note').textContent = formData.originalText; saveAiButton.hidden = true; saveOriginalNoteButton.hidden = false; document.querySelector('#edit-ai-activity').textContent = 'Back to Note'; aiReviewActions.hidden = false;
  }
}

function saveReviewedActivity() { if (!pendingActivity?.aiAnalysis) return; CropPassport.saveActivity({ id: CropPassport.createActivityId(), harvestId: harvest.id, ...pendingActivity, createdAt: new Date().toISOString() }); pendingActivity = null; closeAiReview(); renderActivities(); }
function saveOriginalFromReview() { if (!pendingActivity) return; saveOriginalActivity(pendingActivity); pendingActivity = null; closeAiReview(); renderActivities(); }
